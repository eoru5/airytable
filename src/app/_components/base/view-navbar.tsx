...
export default function ViewNavbar({
  ...
  return (
    <div className="flex border-b-1 border-neutral-300 bg-white px-3 py-2 text-sm font-light">
      <Menu>
        <MenuButton>
          <div className={`flex cursor-pointer items-center gap-1 rounded-sm px-3 py-1 transition duration-150 ${sorting.length > 0 ?'bg-emerald-100 hover:border-1' : 'hover:bg-neutral-200'}`}>
            <svg
              ...
              {sorting.length > 0 ? (
                <>
                  {fields
                    .filter(
                      (f) =>
                        sorting.findIndex((s) => s.id === f.id.toString()) !==
                        -1,
                    )
                    .map((f) => (
                      <SortOption
